 ...
 *
 * All existing articles are imported as status='published'.
 * Safe to run multiple times (uses ON CONFLICT DO UPDATE on slug).
 */
...
async function seed() {
  ...
  for (const a of articles) {
    try {
      const publishedAt = a.dateISO ? new Date(a.dateISO) : new Date();
      await query(`
        INSERT INTO articles (slug, title, excerpt, body, category, category_slug, tags, date_iso, date_human,
          reading_time, hero_image, hero_alt, og_image, opener_type, faq_count, faqs, backlink_type,
        ...
        ON CONFLICT (slug) DO UPDATE SET
          ...
          category_slug=EXCLUDED.category_slug, tags=EXCLUDED.tags, hero_image=EXCLUDED.hero_image,
      ...
      `, [
        ...
        a.hasAffiliateLinks, a.asins, a.wordCount, publishedAt
      ]);
      inserted++;
    } catch (err) {
  ...
  console.log(`[seed] Inserted/updated ${inserted}/${articles.length} articles as status='published'`);
  await close();
}
