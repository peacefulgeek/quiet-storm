...
async function deepRefreshFromAnthropic(article) {
...
export async function refreshQuarterly() {
  ...
  for (const a of rows) {
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      refreshed = await deepRefreshFromAnthropic(a);
    ...
    if (passed) {
      await query('UPDATE articles SET body = $1, last_refreshed_90d = NOW() WHERE id = $2', [refreshed, a.id]);
      console.log(`[refresh-quarterly] refreshed ${a.slug}`);
    } else {
